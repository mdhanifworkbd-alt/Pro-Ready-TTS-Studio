...
import { AVAILABLE_VOICES, PRE_GENERATED_TOKENS } from './constants';
...
const App: React.FC = () => {
    ...
    const saveTokens = useCallback((updatedTokens: Token[]) => {
    ...
    useEffect(() => {
        try {
            const storedTokensRaw = localStorage.getItem('tts_tokens');
            const existingTokens: Token[] = storedTokensRaw ? JSON.parse(storedTokensRaw) : [];
            const existingTokenSet = new Set(existingTokens.map(t => t.token));

            const initialTimestamp = Date.now();
            const newTokensFromList = PRE_GENERATED_TOKENS
                .filter(token => !existingTokenSet.has(token))
                .map(token => ({
                    token,
                    txid: 'pre-generated',
            ...
            if (newTokensFromList.length > 0) {
                const updatedTokens = [...existingTokens, ...newTokensFromList];
                saveTokens(updatedTokens);
            } else {
                // If no new tokens, just set the existing ones to state
                setTokens(existingTokens);
            }

        } catch (e) {
            console.error("Failed to initialize or merge tokens:", e);
            // If local storage is corrupt, initialize with the full list
            const initialTokens = PRE_GENERATED_TOKENS.map(token => ({
                token,
                txid: 'pre-generated',
                timestamp: Date.now(),
                status: 'new' as const,
